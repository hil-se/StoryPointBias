...
def acquire(inds, pool, preds_train, train_x, train_y, delta = 5.0, step = 10):
    axs = []
    for i in pool:
...
def active(dataname, treatment = "None", init = 10, step = 10, delta = 5.0):
    ...
    results = []
    while pool:
        model = train(train_x[inds], train_y[inds], treatment = treatment)
...
def train(train_x, train_y, treatment = "None"):
...
if __name__ == "__main__":
    ...
    print(results)
    results.to_csv("../Results/STD_active.csv", index=False)
