...
def comparative_learning(de, train_x, test_x, features):
    ...
    # Train model
    history = de.fit(features, features["Label"], epochs=100, batch_size=10, callbacks=[reduce_lr, checkpoint],
                     verbose=1)
...
def active(dataname, init = 10, step = 10):
    ...
    de = ComparativeModel(encoder=encoder, w_loss="hinge")

    # de.compile(optimizer="SGD", loss=tf.keras.losses.Hinge())
    de.compile(optimizer="adam")

    while n < len(train_data)*2:
        # Comparative learning
        preds_test, preds_train = comparative_learning(de, train_x, test_x, features)
